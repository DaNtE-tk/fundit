...
import {thirdweb, menu, search, metamask} from '../assets';
  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
        ...
        <div className="w-[72px] h-hull rounded-[20px] bg-[#F10086] flex justify-center items-center cursor-pointer">
      ...
      <div className="sm:flex hidden flex-row justify-end gap-4">
        <CustomButton 
          ...
          styles = {address ? 'bg-[#F10086] text-white' : 'bg-[#30E3DF] text-white '}
          handleClick = {()=>{
          ...
          <div className={`absolute top-[60px] right-0 left-0 bg-[#1c1c24] z-10 shadow-secondary py-4 ${!toggleDrawer ? '-translate-y-[100vh]' : 'translate-y-0'} transition-all duration-700`}>
            <ul className='mb-4'> 
              {navlinks.map((link)=>(
                <li 
                  key={link.name}
                  className={`flex p-4 ${isActive === link.name && 'bg-[#3a3a43]'}`} onClick={()=>{
